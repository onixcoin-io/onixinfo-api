...
class ContractController extends Controller {
  async summary() {
    ...
    ctx.body = {
      ...
      ...summary.type === 'orc20' ? {
        orc20: {
          name: summary.orc20.name,
          symbol: summary.orc20.symbol,
          decimals: summary.orc20.decimals,
          totalSupply: summary.orc20.totalSupply.toString(),
          version: summary.orc20.version,
          holders: summary.orc20.holders,
          transactions: summary.orc20.transactions
        }
      } : {},
      ...summary.type === 'orc721' ? {
        orc721: {
          name: summary.orc721.name,
          symbol: summary.orc721.symbol,
          totalSupply: summary.orc721.totalSupply.toString()
        }
      } : {},
  ...
  async orc20BalanceHistory() {
    ...
    let {totalCount, transactions} = await ctx.service.orc20.getORC20BalanceHistory([ctx.state.contract.contractAddress], tokenAddress)
    ctx.body = {
      totalCount,
      transactions: transactions.map(tx => ({
        id: tx.id.toString('hex'),
