...
class TransactionService extends Service {
  async getTransaction(id) {
    const {
      ...
      Contract, Orc20: ORC20, Orc721: ORC721,
      where, col
    } = this.ctx.model
    const {in: $in} = this.app.Sequelize.Op
    const {Address: RawAddress} = this.app.onixinfo.lib
        ...
        if (output.evmReceipt) {
          ...
          outputObject.evmReceipt.logs = eventLogs.filter(log => log.receiptId === output.evmReceipt._id).map(log => ({
            ...
            ...log.orc20 ? {
              orc20: {
                name: log.orc20.name,
                symbol: log.orc20.symbol,
                decimals: log.orc20.decimals
              }
            } : {},
            ...log.orc721 ? {
              orc721: {
                name: log.orc721.name,
                symbol: log.orc721.symbol
              }
            } : {}
          }))
        }
  ...
  async getRawTransaction(id) {
    ...
    const {Transaction: RawTransaction, Input, Output, OutputScript} = this.app.onixinfo.lib
  ...
  async sendRawTransaction(data) {
    let client = new this.app.onixinfo.rpc(this.app.config.onixinfo.rpc)
  ...
  async transformTransaction(transaction, {brief = false} = {}) {
    ...
    let [orc20TokenTransfers, orc20TokenUnconfirmedTransfers, orc721TokenTransfers] = await Promise.all([
      this.transformORC20Transfers(transaction.outputs),
      confirmations === 0 ? this.transformORC20UnconfirmedTransfers(transaction.outputs) : undefined,
      this.transformORC721Transfers(transaction.outputs)
    ])
    ...
    return {
      ...
      orc20TokenTransfers,
      orc20TokenUnconfirmedTransfers,
      orc721TokenTransfers
    }
  }

  transformInput(input, index, transaction, {brief}) {
    const {InputScript, OutputScript} = this.app.onixinfo.lib
  ...
  transformOutput(output, index, {brief}) {
    const {OutputScript} = this.app.onixinfo.lib
  ...
  async transformORC20Transfers(outputs) {
    const TransferABI = this.app.onixinfo.lib.Solidity.orc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
    for (let output of outputs) {
      if (output.evmReceipt) {
        for (let {addressHex, topics, data, orc20} of output.evmReceipt.logs) {
          if (orc20 && topics.length === 3 && Buffer.compare(topics[0], TransferABI.id) === 0 && data.length === 32) {
  ...
  async transformORC721Transfers(outputs) {
    const TransferABI = this.app.onixinfo.lib.Solidity.orc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
    for (let output of outputs) {
      if (output.evmReceipt) {
        for (let {addressHex, topics, orc721} of output.evmReceipt.logs) {
          if (orc721 && topics.length === 4 && Buffer.compare(topics[0], TransferABI.id) === 0) {
            let [from, to] = await this.ctx.service.contract.transformHexAddresses([topics[1].slice(12), topics[2].slice(12)])
            result.push({
              address: addressHex.toString('hex'),
              addressHex: addressHex.toString('hex'),
              name: orc721.name,
              symbol: orc721.symbol,
              ...from && typeof from === 'object' ? {from: from.hex.toString('hex'), fromHex: from.hex.toString('hex')} : {from},
              ...to && typeof to === 'object' ? {to: to.hex.toString('hex'), toHex: to.hex.toString('hex')} : {to},
  ...
  async getContractTransaction(receiptId) {
    const {Address: RawAddress, OutputScript} = this.app.onixinfo.lib
    const {
