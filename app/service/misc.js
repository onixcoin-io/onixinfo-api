...
class MiscService extends Service {
  async classify(id) {
    ...
    const {Block, Transaction, Contract, Orc20: ORC20, where, fn, literal} = db
    ...
    let orc20Results = (await ORC20.findAll({
      where: {
        [$or]: [
      ...
      transaction
    })).map(orc20 => orc20.contractAddress)
    if (orc20Results.length === 0) {
      orc20Results = (await ORC20.findAll({
        where: {
          [$or]: [
    ...
    if (orc20Results.length) {
      let [{addressHex}] = await db.query(sql`
        SELECT contract.address_string AS address, contract.address AS addressHex FROM (
          SELECT contract_address FROM orc20_statistics
          WHERE contract_address IN ${orc20Results}
          ORDER BY transactions DESC LIMIT 1
        ) orc20_balance
        INNER JOIN contract ON contract.address = orc20_balance.contract_address
