...
class AddressService extends Service {
  async getAddressSummary(addressIds, p2pkhAddressIds, rawAddresses) {
    const {Address} = this.ctx.app.onixinfo.lib
    ...
    let [
      ...
      orc20Service.getAllORC20Balances(hexAddresses),
      orc721Service.getAllORC721Balances(hexAddresses),
  ...
  async getAddressTransactionCount(addressIds, rawAddresses) {
    const {Address: RawAddress, Solidity} = this.app.onixinfo.lib
  ...
  async getAddressORC20TokenTransactionCount(rawAddresses, token) {
    const {Address, Solidity} = this.app.onixinfo.lib
    const TransferABI = Solidity.orc20ABIs.find(abi => abi.name === 'Transfer')
  ...
  async getAddressORC20TokenTransactions(rawAddresses, token) {
    const {Address, Solidity} = this.app.onixinfo.lib
    const TransferABI = Solidity.orc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    ...
    let topicAddresses = rawAddresses
      .filter(address => address.type === Address.PAY_TO_PUBLIC_KEY_HASH)
      .map(address => Buffer.concat([Buffer.alloc(12), address.data]))
    let totalCount = await this.getAddressORC20TokenTransactionCount(rawAddresses, token)
    let transactions = await db.query(sql`
      SELECT
      ...
      INNER JOIN orc20 ON orc20.contract_address = log.address
  ...
  async getAddressORC20TokenMempoolTransactions(rawAddresses, token) {
        ...
        },
        {
          ...
          include: [{
            ...
            include: [{
              ...
              as: 'contract',
              required: true,
              where: {address: token.contractAddress, type: 'orc20'},
              attributes: []
            }]
          }]
        },
