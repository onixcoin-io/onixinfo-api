...
class ORC20Service extends Service {
  async listORC20Tokens() {
    const db = this.ctx.model
    const {Orc20Statistics: ORC20Statistics} = db
    const {sql} = this.ctx.helper
    const {gt: $gt} = this.app.Sequelize.Op
    let {limit, offset} = this.ctx.state.pagination

    let totalCount = await ORC20Statistics.count({
    ...
    let list = await db.query(sql`
      SELECT
        ...
        orc20.name AS name, orc20.symbol AS symbol, orc20.decimals AS decimals, orc20.total_supply AS totalSupply,
        orc20.version AS version,
        list.holders AS holders,
        list.transactions AS transactions
      FROM (
        SELECT contract_address, holders, transactions FROM orc20_statistics
        WHERE transactions > 0
        ORDER BY transactions DESC
        LIMIT ${offset}, ${limit}
      ) list
      INNER JOIN orc20 USING (contract_address)
  ...
  async getAllORC20Balances(hexAddresses) {
    if (hexAddresses.length === 0) {
      return []
    }
    const {OutputScript, Solidity} = this.app.onixinfo.lib
    const transferABI = Solidity.orc20ABIs.find(abi => abi.name === 'transfer')
    const {
      Address, TransactionOutput,
      Contract, EvmReceipt: EVMReceipt, Orc20: ORC20, Orc20Balance: ORC20Balance,
      where, col
    } = this.ctx.model
    const {in: $in} = this.app.Sequelize.Op
    let list = await ORC20.findAll({
    ...
    let mapping = new Map(list.map(item => [
      item.contract.addressString,
      {
        ...
        symbol: item.symbol,
        decimals: item.decimals,
        balance: item.contract.orc20Balances.map(({balance}) => balance).reduce((x, y) => x + y),
        unconfirmed: {
          received: 0n,
          sent: 0n
        }
      ...
      if (byteCode.length === 68
        ...
        if (mapping.has(item.output.address.contract.addressString)) {
          ...
          data = {
            ...
            name: item.output.address.contract.orc20.name,
            symbol: item.output.address.contract.orc20.symbol,
            decimals: item.output.address.contract.orc20.decimals,
            balance: 0n,
            unconfirmed: {
              received: 0n,
              sent: 0n
  ...
  async getORC20Balance(rawAddresses, tokenAddress) {
    const {Address: RawAddress, OutputScript, Solidity} = this.app.onixinfo.lib
    const transferABI = Solidity.orc20ABIs.find(abi => abi.name === 'transfer')
    const {
      Address, TransactionOutput,
      Contract, EvmReceipt: EVMReceipt, Orc20: ORC20, Orc20Balance: ORC20Balance,
    ...
    let token = await ORC20.findOne({
      where: {contractAddress: tokenAddress},
      attributes: ['name', 'symbol', 'decimals'],
      transaction: this.ctx.state.transaction
    })
    let list = await ORC20Balance.findAll({
  ...
  async getORC20BalanceHistory(addresses, tokenAddress) {
    const TransferABI = this.app.onixinfo.lib.Solidity.orc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    const {
      Header, Transaction,
      EvmReceipt: EVMReceipt, EvmReceiptLog: EVMReceiptLog,
      Contract, Orc20: ORC20, Orc20Balance: ORC20Balance,
      literal
    } = db
    ...
    let [{totalCount}] = await db.query(sql`
      ...
      FROM evm_receipt receipt, evm_receipt_log log, orc20
      WHERE receipt._id = log.receipt_id AND log.address = orc20.contract_address AND ${{raw: logFilter}}
    `, {type: db.QueryTypes.SELECT, transaction: this.ctx.state.transaction})
    if (totalCount === 0) {
      return {totalCount: 0, transactions: []}
    }
    let ids = (await db.query(sql`
      ...
      INNER JOIN (
        SELECT DISTINCT(receipt.transaction_id) AS id FROM evm_receipt receipt, evm_receipt_log log, orc20
        WHERE receipt._id = log.receipt_id AND log.address = orc20.contract_address AND ${{raw: logFilter}}
    ...
    if (list.length > 0) {
      let intialBalanceList = await ORC20Balance.findAll({
        where: {
      ...
      for (let log of logs) {
        ...
        if (item) {
          ...
          result.tokens.push({
            ...
            name: log.orc20.name.toString(),
            symbol: log.orc20.symbol.toString(),
            decimals: log.orc20.decimals,
            amount: delta
          })
        }
      }
  ...
  async getAllORC20TokenTransactions() {
    const TransferABI = this.app.onixinfo.lib.Solidity.orc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    let {limit, offset, reversed = true} = this.ctx.state.pagination
    ...
    let [{totalCount}] = await db.query(sql`
      ...
      FROM orc20, evm_receipt_log log
      WHERE orc20.contract_address = log.address AND log.topic1 = ${TransferABI.id}
    `, {type: db.QueryTypes.SELECT, transaction: this.ctx.state.transaction})
    let transactions = await db.query(sql`
      SELECT
        ...
        orc20.name AS name,
        orc20.symbol AS symbol,
        orc20.decimals AS decimals,
        evm_receipt_log.topic2 AS topic2,
        evm_receipt_log.topic3 AS topic3,
        evm_receipt_log.data AS data
      FROM (
        SELECT log._id AS _id FROM orc20, evm_receipt_log log
        WHERE orc20.contract_address = log.address AND log.topic1 = ${TransferABI.id}
      ...
      INNER JOIN orc20 ON orc20.contract_address = evm_receipt_log.address
  ...
  async getORC20TokenTransactions(contractAddress) {
    const TransferABI = this.app.onixinfo.lib.Solidity.orc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
  ...
  async getORC20TokenRichList(contractAddress) {
    const db = this.ctx.model
    const {Orc20Balance: ORC20Balance} = db
    const {ne: $ne} = this.app.Sequelize.Op
    let {limit, offset} = this.ctx.state.pagination

    let totalCount = await ORC20Balance.count({
  ...
  async updateORC20Statistics() {
    const TransferABI = this.app.onixinfo.lib.Solidity.orc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {Orc20: ORC20, Orc20Statistics: ORC20Statistics} = db
    const {sql} = this.ctx.helper
    let transaction = await db.transaction()
    try {
      let result = (await ORC20.findAll({attributes: ['contractAddress'], transaction})).map(
        ({contractAddress}) => ({contractAddress, holders: 0, transactions: 0})
      )
      let balanceResults = await db.query(sql`
        SELECT contract_address AS contractAddress, COUNT(*) AS count FROM orc20_balance
      ...
      await db.query(sql`DELETE FROM orc20_statistics`, {transaction})
      await ORC20Statistics.bulkCreate(result, {validate: false, transaction, logging: false})
