...
class ContractService extends Service {
  async getContractAddresses(list) {
    const {Address} = this.app.onixinfo.lib
  ...
  async getContractSummary(contractAddress, addressIds) {
    const {Contract, Orc20: ORC20, Orc20Statistics: ORC20Statistics, Orc721: ORC721} = this.ctx.model
    const {balance: balanceService, orc20: orc20Service, orc721: orc721Service} = this.ctx.service
    ...
    let [
      ...
      orc20Service.getAllORC20Balances([contractAddress]),
      orc721Service.getAllORC721Balances([contractAddress]),
    ...
    return {
      ...
      ...contract.type === 'orc20' ? {
        orc20: {
          name: contract.orc20.name,
          symbol: contract.orc20.symbol,
          decimals: contract.orc20.decimals,
          totalSupply: contract.orc20.totalSupply,
          version: contract.orc20.version,
          holders: contract.orc20.statistics.holders,
          transactions: contract.orc20.statistics.transactions
        }
      } : {},
      ...contract.type === 'orc721' ? {
        orc721: {
          name: contract.orc721.name,
          symbol: contract.orc721.symbol,
          totalSupply: contract.orc721.totalSupply
        }
      } : {},
    ...
    let [{count}] = await db.query(sql`
      SELECT COUNT(*) AS count FROM (
        ...
        WHERE log.receipt_id = receipt._id
          AND ${this.ctx.service.block.getRawBlockFilter('receipt.block_height')}
          AND contract.address = log.address AND contract.type IN ('orc20', 'orc721')
          AND log.topic1 = ${TransferABI.id}
          AND (log.topic2 = ${topic} OR log.topic3 = ${topic})
          AND (
            (contract.type = 'orc20' AND log.topic3 IS NOT NULL AND log.topic4 IS NULL)
            OR (contract.type = 'orc721' AND log.topic4 IS NOT NULL)
          )
      ) list
  ...
  async getContractBasicTransactions(contractAddress) {
    const {Address, OutputScript} = this.app.onixinfo.lib
    const {
